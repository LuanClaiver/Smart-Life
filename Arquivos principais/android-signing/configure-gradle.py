...
"""
    ...
    release_match = re.search(release_pattern, configured)
    if not release_match:
        raise RuntimeError("Bloco buildTypes.release não encontrado no build.gradle")
    release_line = release_match.group(0)
    release_indent = release_match.group(1)
    signing_line = f"{release_indent}    signingConfig signingConfigs.release"
    if "signingConfig signingConfigs.release" not in configured:
        configured = configured.replace(release_line, release_line + "\n" + signing_line, 1)
